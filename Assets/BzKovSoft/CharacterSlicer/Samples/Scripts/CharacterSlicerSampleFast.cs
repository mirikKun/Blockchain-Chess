...
#pragma warning restore 0649
		...
		public void PlaneSlice(Transform planeTransform)
		{
			Debug.Log("11________-");

			int sliceId = SliceIdProvider.GetNewSliceId();
			float currentSliceTime = Time.time;
			// if ((sliceId == 0 & _lastSliceTime + delayBetweenSlices > currentSliceTime) |
			//     (sliceId != 0 & _sliceId == sliceId))
			// {
			...
			Vector3 normal = planeTransform.up.normalized;
			Plane plane = new Plane(normal, point);
			Debug.Log("22________-");
		...
		private void ConvertToRagdoll(GameObject resultNeg, GameObject resultPos, LazyActionRunner lazyRunnerNeg, LazyActionRunner lazyRunnerPos)
		{
			...
			// Vector3 velocityContinue = animator.velocity;
			 ...
			 angularVelocityContinue=Vector3.zero;
			 
			var animSliceConfigurationPos = resultPos.GetComponent<AnimSliceConfiguration>();
			var animSliceConfigurationNeg = resultNeg.GetComponent<AnimSliceConfiguration>();
			Quaternion posRotation=Quaternion.Euler(resultPos.transform.eulerAngles);
			 Vector3 velocityContinue = posRotation*animSliceConfigurationPos.velocity;
			// Vector3 angularVelocityContinue = posRotation*animSliceConfiguration.angularVelocity;

		
			ConvertToRagdoll(resultNeg, velocityContinue, angularVelocityContinue, animSliceConfigurationNeg.childBone,
					animSliceConfigurationNeg.rootBone, animSliceConfigurationNeg.mainSliceType);

			ConvertToRagdoll(resultPos, velocityContinue, angularVelocityContinue, animSliceConfigurationPos.childBone, animSliceConfigurationPos.rootBone,
					animSliceConfigurationPos.cuttedSliceType);
		...
		private void ConvertToRagdoll(GameObject go, Vector3 velocityContinue, Vector3 angularVelocityContinue,Transform childBone,Transform rootBone,AnimSliceType animSliceType)
		{
			Profiler.BeginSample("ConvertToRagdoll");
			...
			if(animSliceType!=AnimSliceType.Animation)
			{
			...
			if(animSliceType==AnimSliceType.Ragdoll)
			{
			...
			if(animSliceType==AnimSliceType.Ragdoll)
			{
				if (childBone && rootBone&&childBone.GetComponent<Rigidbody>())
				{
				

					childBone.parent = rootBone.parent;
					rootBone.parent = childBone;
				}
			}
			Profiler.EndSample();
		}
